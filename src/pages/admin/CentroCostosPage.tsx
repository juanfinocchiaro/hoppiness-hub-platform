...
// ─── Derived types ───
type ItemCartaRow = Tables<'items_carta'> & {
...
type GruposMutations = ReturnType<typeof useGruposOpcionalesMutations>;
...
interface PriceChange extends EI {
  np: number;
  nfc: number;
  nm: number;
  delta: number;
...
const calcSugerido = (costo: number, fcObj: number) =>
...
function enrich(items: ItemCartaRow[]): EI[] {
  return items.map((it) => {
...
function groupByCat(items: EI[], categorias?: Tables<'menu_categorias'>[]): CG[] {
...
export default function CentroCostosPage() {
  ...
  const cmvCats = useMemo(
    () =>
      rdoCategories?.filter(
        (c) =>
          c.level === 3 && (c.parent_code?.startsWith('cmv') || c.code?.startsWith('cmv')),
      ) || [],
    [rdoCategories],
  );
  ...
  const tabs: { id: Tab; label: string; icon: LucideIcon; count?: number }[] = [
    ...
    { id: 'simulador', label: 'Simulador', icon: Calculator },
    {
      ...
      icon: DollarSign,
      count: Object.keys(pending).length || undefined,
    },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <PageHeader
          ...
          subtitle="Análisis de márgenes, simulación y ajuste de precios"
        />
        <div className="flex items-center gap-2">
          ...
          >
            <RefreshCw
              className={`w-4 h-4 mr-2 ${recalculating || isFetching ? 'animate-spin' : ''}`}
            />{' '}
      ...
      <ConfirmDialog
        open={!!delItem}
        onOpenChange={() => setDelItem(null)}
        title="Eliminar item"
        description={`¿Eliminar "${delItem?.nombre}"?`}
        confirmLabel="Eliminar"
        variant="destructive"
        onConfirm={async () => {
          await mutations.softDelete.mutateAsync(delItem.id);
          setDelItem(null);
        }}
      />
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <Card className={gs.sinP > 0 ? 'border-yellow-300' : ''}>
          ...
          <CardContent>
            <p
              className={`text-2xl font-bold ${gs.sinP > 0 ? 'text-yellow-600' : 'text-green-600'}`}
            >
              {gs.sinP}
            </p>
          </CardContent>
        ...
        <Card className={gs.sinC > 0 ? 'border-yellow-300' : ''}>
          ...
          <CardContent>
            <p
              className={`text-2xl font-bold ${gs.sinC > 0 ? 'text-yellow-600' : 'text-green-600'}`}
            >
              {gs.sinC}
            </p>
          </CardContent>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.map((g) => (
            <div
              key={g.id}
              className={`border rounded-lg overflow-hidden ${g.hidden ? 'border-amber-300' : ''}`}
            >
              <button
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`font-semibold text-sm ${g.hidden ? 'text-amber-700 dark:text-amber-400' : ''}`}
                  >
                    {g.nombre}
                  </span>
                  ...
                  <TableHeader>
                    <TableRow className="text-xs">
                      ...
                      <TableHead className="text-right">
                        <span>P. Carta</span>{' '}
                        <span className="text-muted-foreground font-normal">(c/IVA)</span>
                      </TableHead>
                      <TableHead className="text-right">
                        <span>P. Neto</span>{' '}
                        <span className="text-muted-foreground font-normal">(s/IVA)</span>
                      </TableHead>
                          ...
                          >
                            <TableCell>
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  ...
                                  {!i.hasComp && (
                                    <p className="text-xs text-yellow-600">⚠ Sin composición</p>
                                  )}
                                </div>
                              </div>
                            </TableCell>
                            <TableCell className="text-right font-mono text-sm">
                              {i.costo > 0 ? fmt(i.costo) : '—'}
                            </TableCell>
                            <TableCell className="text-right font-mono text-sm">
                              {fmt(i.precio)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-sm text-muted-foreground">
                              {i.hasPrice ? fmt(i.pNeto) : '—'}
                            </TableCell>
                            <TableCell className="text-right font-mono text-sm text-muted-foreground">
                              {fmtPct(i.fcObj)}
                            </TableCell>
                            <TableCell className="text-right">
                              {i.hasComp && i.hasPrice ? (
                                <Badge variant={badgeVar[i.color]}>{fmtPct(i.fc)}</Badge>
                              ) : (
                                '—'
                              )}
                            </TableCell>
                            <TableCell className="text-right font-mono text-sm">
                              {i.hasComp && i.hasPrice ? (
                                <span className={i.margen > 0 ? 'text-green-600' : 'text-red-600'}>
                                  {fmt(i.margen)}
                                </span>
                              ) : (
                                '—'
                            ...
                            <TableCell className="text-right">
                              {i.hasComp && i.pSug > 0
                                ? (() => {
                                    ...
                                    const diffPct =
                                      i.precio > 0 ? ((rounded - i.precio) / i.precio) * 100 : 0;
                                    ...
                                    return (
                                      <div className="flex flex-col items-end gap-0.5">
                                        <span
                                          className={`font-mono text-sm ${gap > 100 ? 'text-red-600 font-semibold' : 'text-muted-foreground'}`}
                                        >
                                          {fmt(i.pSug)}
                                        </span>
                                        <button
                                        ...
                                        {Math.abs(diffAbs) > 0 && (
                                          <span
                                            className={`text-[10px] font-mono ${isUp ? 'text-green-600' : isDown ? 'text-red-600' : 'text-muted-foreground'}`}
                                          >
                                            {isUp ? '+' : ''}
                                            {diffPct.toFixed(1)}% ({isUp ? '+' : ''}
                                            {fmt(diffAbs)})
                                          </span>
                                        )}
                                      </div>
          ...
          <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg flex items-center justify-between">
            ...
            <Badge variant={badgeVar[gs.gColor]} className="text-lg px-4 py-1.5">
              {fmtPct(gs.cmv)}
            </Badge>
          </div>
        </div>
...
function SimuladorTab({
  ...
  const doBulk = () => {
    ...
    workable.forEach((i) => {
      n[i.id] = Math.round(i.precio * (1 + bulkPct / 100));
    });
    setSim(n);
  };
  const doTarget = () => {
    const n: Record<string, number> = {};
    workable.forEach((i) => {
      n[i.id] = Math.round(calcSugerido(i.costo, i.fcObj) / 100) * 100;
    });
    setSim(n);
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="md:col-span-2">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">Simular ajuste de precios</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                />
                <Button size="sm" variant="outline" onClick={doBulk} disabled={!bulkPct}>
                  Subir todo {bulkPct > 0 ? `+${bulkPct}%` : ''}
                </Button>
              </div>
              <Button size="sm" variant="outline" onClick={doTarget}>
                <Target className="w-3.5 h-3.5 mr-1.5" /> Ajustar a CMV objetivo
              </Button>
              <Button size="sm" variant="ghost" onClick={() => setSim({})}>
                Limpiar
              </Button>
            </div>
            <p className="text-xs text-muted-foreground">
              También podés editar precios individuales en la tabla.
            </p>
          </CardContent>
        </Card>
        <Card className={impact.count > 0 ? 'border-primary' : ''}>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs text-muted-foreground">Impacto simulado</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            <div className="flex justify-between text-sm">
              <span>CMV actual:</span>
              <span className="font-mono font-medium">{fmtPct(gs.cmv)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span>CMV simulado:</span>
              <span
                className={`font-mono font-bold ${impact.cmv < gs.cmv ? 'text-green-600' : impact.cmv > gs.cmv ? 'text-red-600' : ''}`}
              >
                {fmtPct(impact.cmv)}
              </span>
            </div>
            ...
            {impact.count > 0 && (
              <Button size="sm" className="w-full mt-2" onClick={onApply}>
                <ArrowRight className="w-3.5 h-3.5 mr-1.5" /> Aplicar estos precios
              </Button>
            )}
          </CardContent>
        </Card>
              ...
              <TableRow>
                <TableCell colSpan={9} className="h-32">
                  <EmptyState
                    icon={Calculator}
                    title="Sin items"
                    description="Necesitás items con precio y composición"
                  />
                </TableCell>
              </TableRow>
            ) : (
              simItems.map((i) => {
                ...
                return (
                  <TableRow key={i.id} className={i.changed ? 'bg-primary/5' : ''}>
                    ...
                    <TableCell className="text-right">
                      <Badge variant={badgeVar[i.color]} className="text-xs">
                        {fmtPct(i.fc)}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <Input
                        type="number"
                        className="h-7 w-[110px] text-right font-mono text-sm ml-auto"
                        value={sim[i.id] ?? i.precio}
                        onChange={(e) =>
                          setSim({ ...sim, [i.id]: parseFloat(e.target.value) || 0 })
                        }
                      />
                    </TableCell>
                    <TableCell className="text-right">
                      <Badge variant={badgeVar[fcColor(i.nfc, i.fcObj)]} className="text-xs">
                        {fmtPct(i.nfc)}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-right font-mono text-sm">
                      {i.changed ? (
                        <span className={i.dm > 0 ? 'text-green-600' : 'text-red-600'}>
                          {i.dm > 0 ? '+' : ''}
                          {fmt(i.dm)}
                        </span>
                      ) : (
    ...
    try {
      for (const c of changes) {
        await mutations.cambiarPrecio.mutateAsync({
          itemId: c.id,
          precioAnterior: c.precio,
          precioNuevo: c.np,
          motivo: 'Ajuste desde Control de Costos',
          userId,
        });
      }
      setPending({});
    ...
    return (
      <div className="py-16">
        <EmptyState
          ...
          description="Usá el Simulador para probar precios y aplicalos acá."
        />
      </div>
    );
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-lg font-semibold">
                {changes.length} cambio{changes.length !== 1 ? 's' : ''} pendiente
                {changes.length !== 1 ? 's' : ''}
              </p>
            ...
            <div className="flex gap-2">
              ...
              <LoadingButton loading={applying} onClick={applyAll}>
              ...
              <TableRow key={c.id}>
                ...
                <TableCell className="text-right font-mono text-sm">
                  <span className={c.delta > 0 ? 'text-green-600' : 'text-red-600'}>
                    {c.delta > 0 ? '+' : ''}
                    {fmt(c.delta)} ({c.deltaPct > 0 ? '+' : ''}
                    {c.deltaPct.toFixed(1)}%)
                  </span>
                </TableCell>
                <TableCell className="text-right text-sm">
                  <span className="text-muted-foreground">{fmtPct(c.fc)}</span> →{' '}
                  <Badge variant={badgeVar[fcColor(c.nfc, c.fcObj)]} className="text-xs">
                    {fmtPct(c.nfc)}
                  </Badge>
                </TableCell>
  ...
  useEffect(() => {
    if (item)
      setForm({
        ...
        rdo_category_code: item.rdo_category_code || '',
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEdit ? 'Editar' : 'Nuevo'} Item de Carta</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <FormSection title="Precio y CMV" icon={DollarSign}>
            ...
            {form.precio_base > 0 && (
              <p className="text-xs text-muted-foreground">
                Precio neto (sin IVA): {fmt(form.precio_base / IVA)}
              </p>
            )}
          </FormSection>
          <StickyActions>
            ...
            <LoadingButton
              ...
              onClick={submit}
              disabled={!form.nombre || !form.precio_base}
            >
...
function ComposicionModal({
  ...
  const updateRow = (i: number, field: string, value: string | number) => {
    ...
    if (field === 'preparacion_id') {
      const p = preparaciones.find((x) => x.id === value);
  ...
  const costoFijo = rows
    .filter((r) => r.cantidad > 0)
    .reduce((t, r) => t + r.cantidad * r._costo, 0);
  ...
  const handleSave = async () => {
    await mutations.saveComposicion.mutateAsync({
      ...
      items: rows
        .filter((r) => r.preparacion_id || r.insumo_id)
        .map((r) => ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Composición: {item.nombre}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
                    ...
                    <div className="flex-1 min-w-0">
                      {row.tipo === 'preparacion' ? (
                        <Select
                          ...
                          onValueChange={(v) =>
                            updateRow(i, 'preparacion_id', v === 'none' ? '' : v)
                          }
                        >
                          <SelectTrigger className="h-7 text-xs">
                            <SelectValue placeholder="Seleccionar..." />
                ...
                {item.precio_base > 0 && (
                  <div className="text-right">
                    ...
                    <Badge
                      variant={
                        badgeVar[
                          fcColor(calcFC(costoTotal, item.precio_base), item.fc_objetivo || 32)
                        ]
                      }
              ...
              >
                <div>
                  <span className="font-mono">
                    {fmt(h.precio_anterior || 0)} → {fmt(h.precio_nuevo)}
                  </span>
