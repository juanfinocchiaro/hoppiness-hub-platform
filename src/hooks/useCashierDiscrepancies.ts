...
export const discrepancyKeys = {
  ...
  stats: (userId: string, branchId?: string) => [...discrepancyKeys.all, 'stats', userId, branchId] as const,
  history: (userId: string, branchId?: string) => [...discrepancyKeys.all, 'history', userId, branchId] as const,
...
export function useBranchDiscrepancyReport(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Ordenar por discrepancia (más negativa primero)
