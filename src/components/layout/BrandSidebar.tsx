...
import {
  LayoutDashboard, Store, Users, Megaphone, MapPin, Plus,
...
import {
  ...
  NavDashboardLink, NavActionButton,
} from './WorkSidebar';
import NewBranchModal from '@/components/admin/NewBranchModal';

/* ── Sortable wrapper for a section ── */
function SortableSection({ id, children, disabled }: { id: string; children: ReactNode; disabled: boolean }) {
...
export function BrandSidebar() {
  ...
  const isMenuEngActive = p.includes('/mimarca/finanzas/insumos') || p.includes('/mimarca/finanzas/proveedores') || p.includes('/mimarca/recetas') || p.includes('/mimarca/carta') || p.includes('/mimarca/categorias-carta') || p === '/mimarca/centro-costos' || p.includes('/mimarca/precios-canal') || p.includes('/mimarca/promociones') || p.includes('/mimarca/codigos-descuento');
  ...
  const sectionMap: Record<string, ReactNode> = useMemo(() => ({
    locales: canSeeLocales ? (
      <NavSectionGroup id="locales" label="Locales" icon={Store} defaultOpen forceOpen={isLocalesActive}>
        {branches?.map((branch) => (
          <NavItemButton key={branch.id} to={`/mimarca/locales/${branch.slug}`} icon={MapPin} label={branch.name} />
        ))}
        {bp.canCreateLocales && (
          <NavActionButton icon={Plus} label="Nueva Sucursal" onClick={() => setShowNewBranchModal(true)} variant="primary" />
        )}
      </NavSectionGroup>
    ) : null,
    ...
    'menu-eng': canSeeMenuEng ? (
      <NavSectionGroup id="menu-eng" label="Producto y Menú" icon={Beef} forceOpen={isMenuEngActive}>
        ...
        {bp.canViewInsumos && <NavItemButton to="/mimarca/precios-canal" icon={Tag} label="Precios por Canal" />}
        ...
        {bp.canViewInsumos && <NavItemButton to="/mimarca/promociones" icon={Tag} label="Promociones" />}
        {bp.canViewInsumos && <NavItemButton to="/mimarca/codigos-descuento" icon={Tag} label="Códigos Descuento" />}
    ...
    'gestion-red': canSeeGestionRed ? (
      <NavSectionGroup id="gestion-red" label="Supervisión de Red" icon={Network} forceOpen={isGestionRedActive}>
        ...
        {bp.canManageMessages && <NavItemButton to="/mimarca/comunicados" icon={Megaphone} label="Comunicados" />}
        {bp.canViewMeetings && <NavItemButton to="/mimarca/reuniones" icon={Calendar} label="Reuniones" />}
      </NavSectionGroup>
    ) : null,

    'modelo-op': canSeeModeloOp ? (
      <NavSectionGroup id="modelo-op" label="Modelo Operativo" icon={Boxes} forceOpen={isModeloOpActive}>
        {bp.canViewConceptosServicio && <NavItemButton to="/mimarca/finanzas/conceptos-servicio" icon={FileText} label="Estructura de Costos" />}
    ...
    admin: canSeeAdmin ? (
      <NavSectionGroup id="admin" label="Administración" icon={Briefcase} forceOpen={isAdminActive}>
        {bp.canViewContactMessages && (
          <NavItemButton to="/mimarca/mensajes" icon={MessageSquare} label="Bandeja de Entrada" badge={unreadCount || undefined} badgeVariant="warning" />
        )}
  ...
  }), [p, branches, bp, isSuperadmin, unreadCount, canSeeLocales, canSeeMenuEng, canSeeGestionRed, canSeeModeloOp, canSeeFinanzas, canSeeAdmin]);
