...
BEGIN
  ...
  INSERT INTO public.facturas_proveedores (
    ...
    'C',
    v_factura_numero,
    -- factura_fecha: último día del mes del canon
    (DATE_TRUNC('month', (NEW.periodo || '-01')::date) + INTERVAL '1 month' - INTERVAL '1 day')::date,
    total_canon,
    total_canon,
    'cuenta_corriente',
    -- fecha_vencimiento: 1ro del mes siguiente al periodo
    (DATE_TRUNC('month', (NEW.periodo || '-01')::date) + INTERVAL '1 month')::date,
    'pendiente',
    total_canon,
    NEW.periodo,
    'normal',
...
-- Also update existing canon invoices to have the 1st of the next month as due date
UPDATE public.facturas_proveedores
SET fecha_vencimiento = (DATE_TRUNC('month', (periodo || '-01')::date) + INTERVAL '1 month')::date
WHERE proveedor_id = '00000000-0000-0000-0000-000000000001'
