-- FIX: Permitir que CUALQUIER usuario con is_hr_role pueda eliminar fichajes
-- Esto reemplaza la policy de delete para incluir superadmin y managers

-- Primero verificar que sos superadmin (esto va a fallar si no lo sos)
-- SELECT is_superadmin(auth.uid());

-- Drop y recrear la policy de DELETE con la misma lógica que SELECT
...
-- Drop y recrear la policy de UPDATE con la misma lógica que SELECT
...
-- Asegurarse de que las funciones admin existan (redundante pero seguro)
...
BEGIN
  ...
  -- Clear FK references in employee_time_state before deleting
